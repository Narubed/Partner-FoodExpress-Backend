...
exports.findPartnerOne = (req, res) => {
  const id = req.params.id;
  Order.find({ order_partner_id: id })
    .then((data) => {
      if (!data)
        res
          .status(404)
...
exports.update = async (req, res) => {
  ...
  try {
    let upload = multer({ storage: storage }).single("order_partner_image");
    ...
    async function uploadFileEdit(req, res) {
      ...
      let fileMetaData = {
        ...
        parents: ["1aGhJ2R-b2Vc5CjqUs7DclvryvvvKa7Vz"],
      };
      let media = {
      ...
      try {
        ...
        Order.findByIdAndUpdate(
          id,
          { ...req.body, order_partner_image: response.data.id },
          { useFindAndModify: false }
        )
          .then(async (data) => {
            if (!data) {
