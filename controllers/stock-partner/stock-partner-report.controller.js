...
const {
  StockReport,
  validate,
} = require("../../models/stock-partner/stock-partner-report.model");
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const result = await new StockReport({
      ...req.body,
    }).save();
...
exports.findOnePartner = (req, res) => {
  const id = req.params.id;
  StockReport.find({ spr_partner_id: id })
    .then((data) => {
      if (!data)
        res
          .status(404)
...
exports.delete = (req, res) => {
  ...
  StockReport.findByIdAndRemove(id, { useFindAndModify: false })
    .then((data) => {
      if (!data) {
        res.status(404).send({
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const id = req.params.id;
    StockReport.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
      .then((data) => {
        if (!data) {
          res.status(404).send({
